...
if (isset($_POST['apply'])) {
    ...
    $email_length=strlen((string)$email);
        ...
        if($i!=1 || $j!=1){
            $error['apply']="Invalid Email Entry";
        }
    if (empty($firstname)) {
        ...
        $error['apply']="Enter password of atleast 8 characters";
    }
    else if($password_length>=8){
        for($i=0; $i<$password_length; $i++)
        {
            if($password[$i]==" "){
                $error['apply']="Empty spaces in password";
            }
        }
    }
    if ($confirm_password!=$password) {
        $error['apply']="Passwords do not match";
    }
